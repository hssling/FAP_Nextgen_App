...
export const useFamilyActions = (familyId, studentId) => {
    ...
    const updateFamilyMutation = useMutation({
        mutationFn: async ({ updates }) => {
            ...
            if (isOffline) {
                await addToQueue('UPDATE_FAMILY', payload);
                return { id: familyId, ...payload.updates };
            }
    ...
    // --- Archive Family Mutation (soft delete) ---
    const archiveFamilyMutation = useMutation({
        mutationFn: async () => {
            ...
            const payload = { id: familyId, updates };

            if (isOffline) {
                await addToQueue('ARCHIVE_FAMILY', payload);
                return { id: familyId, ...updates };
            }
    ...
    return {
        ...
        archiveFamily: archiveFamilyMutation.mutateAsync,
        ...
        isArchivingFamily: archiveFamilyMutation.isPending
    };
};
